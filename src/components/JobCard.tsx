...
export function JobCard({ job }: { job: Job }) {
  ...
  return (
    <div className="relative break-inside-avoid">
      <div className="absolute left-[-41px] top-5 w-6 h-6 rounded-full border-2 border-primary bg-background z-10 print:sm:block lg:block hidden"></div>
      ...
      <Card className="shadow-md">
        <CardContent className="p-6">
          <div className="text-muted-foreground text-sm mb-1">{since.format("MMM YYYY")} - {job.to ? to.format("MMM YYYY") : "present"}</div>
          <h4 className="text-lg mb-1">{job.company}</h4>
          <h3 className="text-xl font-bold mb-3">{job.position}</h3>
          <RichText className="text-muted-foreground mb-4 rich-text" data={job.description} />
